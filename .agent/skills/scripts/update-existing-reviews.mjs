...
function main() {
    console.log('📂 Scanning reviews in:', REVIEWS_DIR);

    // Check if reviews directory exists
    if (!fs.existsSync(REVIEWS_DIR)) {
        console.error('❌ Error: Reviews directory not found:', REVIEWS_DIR);
        process.exit(1);
    }
    ...
    for (const slug of folders) {
        ...
        if (!fs.existsSync(indexPath)) {
            console.warn(`⚠️  Skipping ${slug}: no index.mdx`);
            continue;
        }
        ...
        if (!title) {
            console.warn(`⚠️  Skipping ${slug}: no title in frontmatter`);
            continue;
        }
